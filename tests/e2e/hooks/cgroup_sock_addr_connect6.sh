...
${BFCLI} ruleset set --dry-run --from-str "chain test BF_HOOK_CGROUP_SOCK_ADDR_CONNECT6 ACCEPT rule meta.pid not 1234 counter DROP"
...
${BFCLI} ruleset set --dry-run --from-str "chain test BF_HOOK_CGROUP_SOCK_ADDR_CONNECT6 ACCEPT rule (meta.pid) in { 1; 1234 } counter DROP"
${BFCLI} ruleset set --dry-run --from-str "chain test BF_HOOK_CGROUP_SOCK_ADDR_CONNECT6 ACCEPT rule (ip6.daddr, meta.pid) in { ::1, 1; ::2, 1234 } counter DROP"
...
${FROM_NS} ${BFCLI} chain set --from-str "chain c BF_HOOK_CGROUP_SOCK_ADDR_CONNECT6{cgpath=${CGROUP_PATH}} ACCEPT rule meta.pid eq 1 counter DROP"
udp6_connect ${HOST_IP6_ADDR} 9990
test "$(get_counter c 0)" = "0"

# meta.pid not
${FROM_NS} ${BFCLI} chain set --from-str "chain c BF_HOOK_CGROUP_SOCK_ADDR_CONNECT6{cgpath=${CGROUP_PATH}} ACCEPT rule meta.pid not 1 counter DROP"
(! udp6_connect ${HOST_IP6_ADDR} 9990)
test "$(get_counter c 0)" = "1"
...
${FROM_NS} ${BFCLI} chain set --from-str "chain c BF_HOOK_CGROUP_SOCK_ADDR_CONNECT6{cgpath=${CGROUP_PATH}} ACCEPT rule (ip6.daddr, meta.pid) in { ${HOST_IP6_ADDR}, 1 } counter DROP"
udp6_connect ${HOST_IP6_ADDR} 9990
test "$(get_counter c 0)" = "0"
